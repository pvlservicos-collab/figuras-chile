...
const STEP_LABEL: Record<string, string> = {
  hero_view:    "Llegó a la página",
  quiz_1:       "Card 2 — Nombre/foto",
  quiz_2:       "Card 3 — Club",
  quiz_3:       "Card 4 — Email",
  confirm:      "Revisa sus datos",
  loading:      "Generó figurita",
  saiu_gerando: "Salió durante generación",
  result_view:  "Vio preview c/ precio",
  result_ok:    "Vio preview c/ precio",
  result_error: "Error en generación",
  checkout:     "Hizo clic en comprar",
  gracias:      "Compró ✓",
  obrigado:     "Compró ✓",
};

const FUNNEL_STEPS = [
  { key: "hero_view",    label: "Llegó a la página",   color: "#64748b" },
  { key: "quiz_1",       label: "Nombre/foto",          color: "#3b82f6" },
  { key: "quiz_2",       label: "Club",                 color: "#3b82f6" },
  { key: "quiz_3",       label: "Club/datos",            color: "#3b82f6" },
  { key: "confirm",      label: "Revisó datos",          color: "#8b5cf6" },
  { key: "loading",      label: "Generó figurita",       color: "#eab308" },
  { key: "saiu_gerando", label: "Salió en generación",  color: "#f97316" },
  { key: "result_view",  label: "Vio el precio",        color: "#22c55e" },
  { key: "result_error", label: "Error en generación",  color: "#ef4444" },
  { key: "checkout",     label: "Hizo clic en comprar", color: "#a855f7" },
  { key: "gracias",      label: "Llegó al gracias",     color: "#059669" },
];
...
function StatusBadge({ status }: { status: string | null }) {
  const MAP: Record<string, [string, string, string]> = {
    pendente:    ["#f1f5f9", "#475569", "Pendiente"],
    pago:        ["#dbeafe", "#1d4ed8", "Pagado"],
    entregue:    ["#d1fae5", "#065f46", "Entregado"],
    recuperado:  ["#ede9fe", "#5b21b6", "Recuperado"],
    recuperacao: ["#ffedd5", "#c2410c", "En Recuperación"],
  };
  const [bg, color, label] = MAP[status || ""] ?? ["#f1f5f9", "#94a3b8", "Sin pedido"];
  return (
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          width: 72, height: 72,
          background: "linear-gradient(135deg, #d40000 0%, #9b0000 100%)",
        ...
        <p style={{ color: "#475569", fontSize: 13, margin: "0 0 32px" }}>Copa del Mundo 2026 — La Roja</p>
        <input
          type="text"
          placeholder=""
          value={value}
        ...
        {error && (
          <p style={{ color: "#ef4444", fontSize: 12, margin: "0 0 10px", fontWeight: 600 }}>
            Nombre no reconocido. Intenta de nuevo.
          </p>
        )}
        <button
          ...
          disabled={loading}
          style={{
            width: "100%", background: "#d40000", border: "none", borderRadius: 10,
...
const NAV_ITEMS: { tab: Tab; label: string; icon: string }[] = [
  { tab: "figurinhas",  label: "Figuritas",    icon: "⭐" },
    ...
    <aside style={{
      ...
      <div style={{ padding: "20px 16px 16px", borderBottom: "1px solid #1e293b" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ color: "#f1f5f9", fontWeight: 800, fontSize: 13, letterSpacing: ".05em" }}>FIGURITA</div>
            <div style={{ color: "#475569", fontSize: 9, letterSpacing: ".08em", textTransform: "uppercase" }}>Panel Admin</div>
          </div>
        </div>
      </div>
        ...
        {NAV_ITEMS.map(item => (
          <button
            ...
            style={{
              ...
              background: tab === item.tab ? "#d40000" : "transparent",
      ...
      <div style={{ padding: "14px 12px", borderTop: "1px solid #1e293b" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <div style={{
            width: 34, height: 34, borderRadius: "50%", background: "#d40000", flexShrink: 0,
  ...
  return (
    <div style={{ padding: 24, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>Todas las figuritas generadas, de la más reciente</p>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 16, boxShadow: "0 1px 3px rgba(0,0,0,.07)", marginBottom: 20 }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ background: "#d40000", color: "#fff", border: "none", borderRadius: 8, padding: "9px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", opacity: loading ? 0.7 : 1, whiteSpace: "nowrap" }}
          >
            {loading ? "..." : "Buscar"}
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", gap: 4 }}>
            {([["all", "Todo"], ["today", "Hoy"], ["yesterday", "Ayer"]] as [DateFilter, string][]).map(([d, label]) => (
              <button
                key={d}
                onClick={() => handleDateFilter(d)}
                style={{
                  padding: "5px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600,
                  border: `1px solid ${dateFilter === d ? "#d40000" : "#e2e8f0"}`,
                  background: dateFilter === d ? "#d40000" : "#f8fafc",
          ...
          {figurinhas.map(f => (
            <div key={f.id} style={{ background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.09)", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ padding: "8px 10px 4px", flex: 1 }}>
                ...
                {f.telefone ? (
                  <span style={{ fontSize: 10, color: "#64748b", display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {f.telefone}
                  </span>
                ) : (
                  <span style={{ fontSize: 10, color: "#94a3b8" }}>Sin número</span>
                )}
                <p style={{ margin: "3px 0 0", fontSize: 9, color: "#94a3b8" }}>
                  {new Date(f.created_at).toLocaleString("es-CL", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                </p>
                {f.price_paid != null && (
                  <p style={{ margin: "3px 0 0", fontSize: 10, fontWeight: 800, color: "#059669" }}>
                    ${(f.price_paid / 100).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}
                  </p>
                )}
              </div>
              <div style={{ padding: "6px 8px 9px", display: "flex", gap: 4 }}>
                <a
                  href={`/api/download?url=${encodeURIComponent(f.sticker_url)}&name=figurita-${(f.nome || "sin-nombre").toLowerCase().replace(/\s+/g, "-")}`}
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button key={p} onClick={() => handlePageChange(p)}
                  style={{ border: "1px solid #e2e8f0", borderRadius: 6, padding: "4px 10px", fontSize: 12, cursor: "pointer", background: p === page ? "#d40000" : "#fff", color: p === page ? "#fff" : "#334155", fontWeight: p === page ? 700 : 400 }}>
                  {p + 1}
                </button>
              );
            })}
...
function MetricasTab() {
      ...
      if (dailyRef.current) {
        dailyInst.current = new Chart(dailyRef.current, {
          ...
          data: {
            labels: dailyLabels,
            datasets: [
              { label: "Oferta Principal", data: last14.map(d => dailyMap.get(d)?.count_a || 0), backgroundColor: "#d4000099", borderColor: "#d40000", borderWidth: 1.5, borderRadius: 3 },
            ],
          },
      ...
      if (funnelRef.current) {
        funnelInst.current = new Chart(funnelRef.current, {
          ...
          data: {
            ...
            datasets: [
              { label: "Oferta Principal", data: FUNNEL_STEPS.map(s => data.funnel.find(f => f.step === s.key || (s.key === "result_view" && f.step === "result_ok"))?.count_a || 0), backgroundColor: "#d4000066", borderColor: "#d40000", borderWidth: 1.5, borderRadius: 3 },
            ],
          },
  ...
  const SUB_TABS = [
    { key: "comp"    as const, label: "Visión General", color: "#0f172a" },
    { key: "segunda" as const, label: "2ª Figurita",   color: "#7c3aed" },
  ...
  return (
    <div style={{ padding: 24, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
            {loading ? "Cargando..." : `Actualiza en ${countdown}s`}
          </p>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => { setPeriod(p.key); setCountdown(60); }}
              style={{ background: period === p.key ? "#d40000" : "#fff", color: period === p.key ? "#fff" : "#64748b", border: `1px solid ${period === p.key ? "#d40000" : "#e2e8f0"}`, borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
              {p.label}
            </button>
          ))}
          <select
            ...
            style={{ background: selectedHour ? "#d40000" : "#fff", color: selectedHour ? "#fff" : "#64748b", border: `1px solid ${selectedHour ? "#d40000" : "#e2e8f0"}`, borderRadius: 6, padding: "5px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600, outline: "none" }}
          >
      ...
      {sub === "comp" && (() => {
        const sa = s.a ?? { total: 0, cta: 0, obrigados: 0, viu_preco: 0 };
        ...
        const ofertaCol = (label: string, color: string, d: { total: number; viu_preco: number; cta: number; obrigados: number }, taxa: number) => (
          <div style={{ background: "#fff", borderRadius: 12, padding: "16px 18px", boxShadow: "0 1px 3px rgba(0,0,0,.07)", flex: 1 }}>
            ...
            {[
              { label: "Sesiones",            value: d.total,      sub: "emails capturados" },
              { label: "Llegaron al precio",   value: d.viu_preco,  sub: "vieron el precio final" },
              { label: "Hicieron clic",        value: d.cta,        sub: "clic en comprar" },
              { label: "Gracias",              value: d.obrigados,  sub: "llegaron a la pág. gracias" },
              { label: "Tasa conversión",      value: `${taxa}%`,   sub: "sesiones → gracias" },
            ].map(c => (
        ...
        return (
          <>
            <div style={{ display: "flex", gap: 16, marginBottom: 24 }}>
              {ofertaCol("Oferta Principal", "#d40000", sa, taxaA)}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
              <div style={chartBox}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", margin: "0 0 14px" }}>📈 Sesiones por día (últimos 14 días)</h3>
                <canvas ref={dailyRef} style={{ maxHeight: 220 }} />
              </div>
              <div style={chartBox}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", margin: "0 0 14px" }}>🚪 Embudo — dónde salen</h3>
      ...
      {sub === "segunda" && (() => {
        ...
        const steps = [
          { label: "Hicieron clic",       n: seg.cliques,   color: "#d40000", pct: 100 },
          { label: "Iniciaron el quiz",    n: seg.starts,    color: "#6366f1", pct: seg.starts    / base * 100 },
          { label: "Llegaron al precio",   n: seg.viu_preco, color: "#8b5cf6", pct: seg.viu_preco / base * 100 },
          { label: "Hicieron clic comprar",n: seg.cta,       color: "#a855f7", pct: seg.cta       / base * 100 },
          { label: "Gracias",              n: seg.obrigados, color: "#059669", pct: seg.obrigados / base * 100 },
        ];
        return (
          <div style={{ background: "#fff", borderRadius: 12, padding: 18, boxShadow: "0 1px 3px rgba(0,0,0,.07)" }}>
            <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#0f172a" }}>Embudo de la 2ª Figurita</p>
      ...
      {sub === "pets" && (() => {
        ...
        const steps = [
          { label: "Sesiones",           n: sp.total,      color: "#16a34a", pct: 100 },
          { label: "Llegaron al precio", n: sp.viu_preco,  color: "#15803d", pct: sp.viu_preco / base * 100 },
          { label: "Hicieron clic",      n: sp.cta,        color: "#166534", pct: sp.cta       / base * 100 },
          { label: "Gracias",            n: sp.obrigados,  color: "#14532d", pct: sp.obrigados / base * 100 },
        ];
        return (
          <>
            <div style={{ background: "#fff", borderRadius: 12, padding: "16px 18px", boxShadow: "0 1px 3px rgba(0,0,0,.07)", marginBottom: 24 }}>
              ...
              {[
                { label: "Sesiones",           value: sp.total,       sub: "emails capturados" },
                { label: "Llegaron al precio", value: sp.viu_preco,   sub: "vieron el precio" },
                { label: "Hicieron clic",      value: sp.cta,         sub: "clic en comprar" },
                { label: "Gracias",            value: sp.obrigados,   sub: "llegaron a la pág. gracias" },
                { label: "Tasa conversión",    value: `${taxaPets}%`, sub: "sesiones → gracias" },
              ].map(c => (
                <div key={c.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "7px 0", borderBottom: "1px solid #f1f5f9" }}>
                  <span style={{ fontSize: 12, color: "#475569" }}>{c.label}</span>
                  <div style={{ textAlign: "right" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: 18, boxShadow: "0 1px 3px rgba(0,0,0,.07)" }}>
              <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#0f172a" }}>Embudo Pets</p>
              <div style={{ display: "flex", alignItems: "stretch", background: "#f8fafc", borderRadius: 10, overflow: "hidden", border: "1px solid #e2e8f0" }}>
                {steps.map((step, i) => (
                  <div key={step.label} style={{ flex: 1, padding: "14px 10px", borderLeft: i > 0 ? "1px solid #e2e8f0" : "none" }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: "#64748b", marginBottom: 8, textTransform: "uppercase", letterSpacing: ".04em" }}>{step.label}</div>
    ...
    <div style={{ padding: 24, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
            {lastUpdate
              ? `Actualizado a las ${lastUpdate.toLocaleTimeString("es-CL")} — actualiza en ${countdown}s`
              : "Cargando..."}
          </p>
        </div>
        <button
          onClick={fetchStats}
          disabled={loading}
          style={{ background: "#d40000", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", opacity: loading ? 0.7 : 1 }}
        >
          {loading ? "..." : "↺ Actualizar"}
        </button>
      </div>
      ...
      {apiStats && apiStats.perKey.length > 0 ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 12, marginBottom: 24 }}>
          {apiStats.perKey.map(k => (
            <div key={k.api_key_used} style={{ background: "#fff", borderRadius: 12, padding: 18, boxShadow: "0 1px 3px rgba(0,0,0,.07)" }}>
              ...
              <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2, marginBottom: 12 }}>generaciones totales</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <div style={{ fontSize: 14, color: "#334155" }}>
                  ...
                  <span style={{ color: "#94a3b8", fontSize: 11 }}> promedio</span>
                </div>
        ...
        <div style={{ background: "#fff", borderRadius: 12, padding: 24, marginBottom: 24, color: "#94a3b8", fontSize: 13, boxShadow: "0 1px 3px rgba(0,0,0,.07)" }}>
          Ningún dato de API disponible aún.
        </div>
      )}
      ...
      {apiStats && apiStats.recentes.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 12, padding: 18, boxShadow: "0 1px 3px rgba(0,0,0,.07)" }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", margin: "0 0 14px" }}>
            Últimas generaciones ({apiStats.recentes.length})
          </h3>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr>
                  {["Nombre", "Email", "API Key", "Tiempo", "Fecha"].map(h => (
                ...
                {apiStats.recentes.map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <td style={{ padding: "8px 12px" }}>
                      <span style={{ background: "#fde8e8", color: "#9b0000", borderRadius: 99, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>
                        Key {r.api_key_used}
                      </span>
                    </td>
                    <td style={{ padding: "8px 12px" }}>
...
const OB_PRODUTOS: Record<string, string> = {
  "3MSNI0": "3× Rifa de la Suerte",
  "3MSNI1": "Paquete Embalaje PDF",
  "3MSNI2": "Poster A4",
  "3MSNI3": "10× Rifa de la Suerte",
  "3MSNI4": "Figurita Edición Especial",
};
...
function groupByPerson(items: OBItem[]): OBGroup[] {
  ...
  for (const item of items) {
    const key = item.email || item.telefone || String(item.id);
    if (!map.has(key)) {
...
function OrderBumpsTab() {
  ...
  const handleAdd = async () => {
    ...
    if (phone.length < 8) { setSaveErr("Teléfono inválido"); return; }
    if (!formProd) { setSaveErr("Selecciona un producto"); return; }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("¿Eliminar este acceso manual?")) return;
  ...
  return (
    <div style={{ padding: 24, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: "0 0 4px" }}>
          Order Bumps{" "}
          {groups.length > 0 && <span style={{ color: "#64748b", fontWeight: 500, fontSize: 14 }}>({groups.length} personas · {items.length} productos)</span>}
        </h2>
        <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>Productos adicionales comprados + accesos manuales</p>
      </div>

      <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,.07)", padding: "20px 24px", marginBottom: 20 }}>
        <p style={{ fontSize: 13, fontWeight: 800, color: "#0f172a", margin: "0 0 14px" }}>Agregar acceso manual</p>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#64748b" }}>Teléfono *</label>
            <input
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#64748b" }}>Nombre (opcional)</label>
            <input
              type="text"
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#64748b" }}>Producto *</label>
            <select
              value={formProd}
          ...
          <button
            ...
            disabled={saving}
            style={{
              background: "#d40000", color: "#fff", border: "none", borderRadius: 8,
        ...
        {saveOk && <p style={{ color: "#059669", fontSize: 12, margin: "8px 0 0" }}>¡Acceso agregado con éxito!</p>}
      </div>

      <div style={{ marginBottom: 12 }}>
        <input
          type="text"
          placeholder="Buscar por teléfono, nombre, email o producto..."
          value={search}
      ...
      {loading ? (
        ...
        <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,.07)", overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 600 }}>
            <thead>
              <tr>
                {["Fecha", "Nombre", "Teléfono", "Email", "Productos comprados"].map(h => (
            ...
            <tbody>
              {groups.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: "center", color: "#94a3b8", padding: 48, fontSize: 13 }}>Ningún order bump aún.</td></tr>
              ) : groups.map(g => (
                <tr key={g.key} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={{ padding: "10px 12px", color: "#94a3b8", fontSize: 11, whiteSpace: "nowrap" }}>
                    {new Date(g.lastDate).toLocaleString("es-CL", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                  </td>
                  ...
                  <td style={{ padding: "10px 12px" }}>
                    {g.telefone ? (
                      <span style={{ color: "#334155", fontWeight: 600 }}>{g.telefone}</span>
...
function LeadsTab() {
  ...
  const downloadCSV = () => {
    const rows = [
      ["Nombre", "Email", "Último paso", "CTA", "Fecha"].join(","),
      ...data.leads.map(l => [
        l.nome || "",
        l.email || "",
        STEP_LABEL[l.step] || l.step,
        l.cta_clicked ? "Sí" : "No",
        new Date(l.updated_at).toLocaleString("es-CL"),
      ].map(v => `"${String(v).replace(/"/g, '""')}"`).join(",")),
    ];
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob(["﻿" + rows.join("\n")], { type: "text/csv;charset=utf-8;" }));
    a.download = "leads-figurita.csv";
    a.click();
  };
    ...
    <div style={{ padding: 24, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>Sesiones con email capturado</p>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <input
            type="text"
            ...
            style={{
              ...
              fontSize: 12, color: "#334155", outline: "none", width: 220,
            }}
          />
          ...
          <button onClick={downloadCSV}
            style={{ background: "#d40000", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
            ⬇ CSV
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,.07)", overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 500 }}>
            <thead>
              <tr>
                {["Fecha", "Nombre", "Email", "Último Paso", "CTA"].map(h => (
                  <th key={h} style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0", padding: "10px 12px", textAlign: "left", fontWeight: 700, color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: ".04em" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {pagedLeads.length === 0 ? (
                ...
                <tr key={l.session_id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={{ padding: "10px 12px", color: "#94a3b8", fontSize: 11, whiteSpace: "nowrap" }}>
                    {new Date(l.updated_at).toLocaleString("es-CL", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                  </td>
                  <td style={{ padding: "10px 12px", color: "#334155", fontWeight: 600 }}>
                  ...
                  <td style={{ padding: "10px 12px" }}>
                    {l.cta_clicked
                      ? <span style={{ padding: "2px 8px", borderRadius: 99, fontSize: 10, fontWeight: 700, background: "#d1fae5", color: "#065f46" }}>✓ Clic</span>
                      : <span style={{ color: "#cbd5e1" }}>—</span>}
                  </td>
                </tr>
              ))}
